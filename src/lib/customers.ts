...
import { MOCK_PAYMENTS, type Payment } from "./payments";
...
export const MOCK_CUSTOMERS: Customer[] = [
  ...
  },
  {
    ...
    verified: true, blocked: false, joined: "2026-03-02", plans: [{ type: "group", reference: "G-013" }], orderCount: 3,
  ...
  },
  {
    ...
    verified: true, blocked: false, joined: "2026-01-08", plans: [{ type: "solo", reference: "SOLO-0042" }], orderCount: 7,
  ...
  },
  {
    ...
    verified: true, blocked: false, joined: "2026-05-11", plans: [{ type: "group", reference: "G-016" }], orderCount: 2,
...
/** Payment submissions for a customer's orders (matched via the order). */
export function getCustomerPayments(customer: Customer): Payment[] {
  return MOCK_PAYMENTS.filter((p) => {
    const order = MOCK_ORDERS.find((o) => o.id === p.orderId);
